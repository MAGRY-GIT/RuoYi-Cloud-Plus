...
public class AlarmLinkageConfigVo implements Serializable {
    ...
    @AutoMapping(source = "linkageConfigs", target = "linkageConfigList"    ,expression = "java(com.cdzeroly.wvp.i1.temp.domain.vo.AlarmLinkageConfigVo.mapLinkageConfigsToList(source.getLinkageConfigs()))")
    private List<AlarmLinkage> linkageConfigList;


    // 新增方法：将 String 转换为 List<AlarmLinkage>
    public static List<AlarmLinkage> mapLinkageConfigsToList(String configs) {
        return JsonUtils.parseObject(configs, new TypeReference<>() {
        });
    }
}
