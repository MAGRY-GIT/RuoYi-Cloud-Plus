package com.cdzeroly.wvp.vmanager.cloudRecord;
...
import com.cdzeroly.wvp.vmanager.cloudRecord.bean.CloudRecordUrl;
...
public class CloudRecordController extends BaseController {
    ...
    public TableDataInfo<CloudRecordItem> openRtpServer(CloudRecordItemBo bo, PageQuery pageQuery) {
        ...
        if (!ObjectUtils.isEmpty(bo.getMediaServerId())) {
            ...
            if (mediaServer == null) {
                throw new ControllerException(ErrorCode.ERROR100.getCode(), "未找到流媒体: " + bo.getMediaServerId());
            }
            mediaServers.add(mediaServer);
        } else {
            mediaServers = mediaServerService.getAllOnlineList();
    ...
    @ResponseBody
    @GetMapping("/zip")
    public void downloadZipFile(CloudRecordItemBo bo) {
        ...
        List<CloudRecordItem> cloudRecordItemList = cloudRecordService.getAllList(bo);
    ...
    @GetMapping("/list-url")
    @Operation(summary = "分页查询云端录像")
    public TableDataInfo<CloudRecordUrl> getListWithUrl(CloudRecordItemBo bo, PageQuery pageQuery
        ...
        TableDataInfo<CloudRecordItem> cloudRecordItemPageInfo = cloudRecordService.getList(bo,pageQuery);
        String finalRemoteHost = remoteHost;
        return cloudRecordItemPageInfo.map(e -> {
            CloudRecordUrl cloudRecordUrl = new CloudRecordUrl();
            cloudRecordUrl.setId(e.getId());
            cloudRecordUrl.setDownloadUrl(finalRemoteHost + "/index/api/downloadFile?file_path=" + e.getFilePath() + "&save_name=" + e.getStream() + "_" + e.getCallId() + "_" + DateUtil.formatDateTime(e.getStartTime()));
            cloudRecordUrl.setPlayUrl(finalRemoteHost + "/index/api/downloadFile?file_path=" + e.getFilePath());
            return cloudRecordUrl;
