...
public class RegionServiceImpl implements IRegionService {
    ...
    public boolean deleteByDeviceId(Long regionDeviceId) {
        ...
        regionMapper.deleteByIds(allChildren);
        return true;
    }
    ...
    @Override
    public TableDataInfo<Region> query(String query, PageQuery pageQuery) {
        if (query != null) {
            query = query.replaceAll("/", "//")
                    .replaceAll("%", "/%")
                    .replaceAll("_", "/_");
        }
        List<Region> regionList =  regionMapper.query(pageQuery.build(),query, null);
        return TableDataInfo.build(regionList);
    }
    ...
    public void update(Region region) {
        ...
        Region regionInDb = regionMapper.selectById(region.getId());
        ...
        regionMapper.insertOrUpdate(region);
        // 发送变化通知
        try {
            // 发送catalog
    ...
    public List<RegionTree> queryForTree(String query, Long parent, Boolean hasChannel) {
        ...
        if (parent != null && hasChannel != null && hasChannel) {
            Region parentRegion = regionMapper.selectById(parent);
    ...
    public void syncFromChannel() {
        ...
        regionMapper.insert(new ArrayList<>(regionMapForVerification.values()));
    }
