package com.cdzeroly.common.core.utils;

import com.cdzeroly.common.core.domain.GeoPoint;
import lombok.extern.slf4j.Slf4j;

import java.util.Base64;

/**
 * @author MGARY
 */
@Slf4j
public class GpsUtil {


    public static GeoPoint Wgs84ToBd09(String xx, String yy) {


        double lng = Double.parseDouble(xx);
        double lat = Double.parseDouble(yy);
        Double[] gcj02 = CoordinateTransformUtil.WGS84ToGCJ02(lng, lat);
        Double[] doubles = CoordinateTransformUtil.GCJ02ToBD09(gcj02[0], gcj02[1]);
        GeoPoint bdPoint= new GeoPoint();
        bdPoint.setLng(doubles[0] );
        bdPoint.setLat(doubles[1]);
        return bdPoint;
    }

    /**
     * BASE64解码
     * @param str
     * @return string
     */
    public static byte[] decode(String str) {
        byte[] bt = null;
        final Base64.Decoder decoder = Base64.getDecoder();
        bt = decoder.decode(str);
        return bt;
    }
}
