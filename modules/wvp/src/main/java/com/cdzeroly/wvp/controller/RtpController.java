...
public class RtpController {
    ...
    @Parameter(name = "ssrc", description = "来源流的SSRC，不传则不校验来源ssrc")
    ...
    public R<OtherRtpSendInfo> openRtpServer(Boolean isSend, @RequestParam(required = false) String ssrc, String callId, String stream, Integer tcpMode, String callBack) {

        log.info("[第三方服务对接->开启收流和获取发流信息] isSend->{}, ssrc->{}, callId->{}, stream->{}, tcpMode->{}, callBack->{}", isSend, ssrc, callId, stream, tcpMode == 0 ? NetProtocol.UDP.name() : "TCP被动", callBack);

        MediaServer mediaServer = mediaServerService.getDefaultMediaServer();
        AssertUtils.isNotNull(mediaServer, "没有可用的MediaServer");
        AssertUtils.notBlack(stream, "stream参数不可为空");
        AssertUtils.isFalse(isSend != null && isSend && callId == null, "isSend为true时，CallID不能为空");
    ...
    public R<Void> closeRtpServer(String stream) {
    ...
    public R<Void> sendRtp(String ssrc, @RequestParam(required = false) String dstIpForAudio, @RequestParam(required = false) String dstIpForVideo, @RequestParam(required = false) Integer dstPortForAudio, @RequestParam(required = false) Integer dstPortForVideo, String app, String stream, String callId, Boolean isUdp, @RequestParam(required = false) Integer ptForAudio, @RequestParam(required = false) Integer ptForVideo) {
        log.info("[第三方服务对接->发送流] " + "ssrc->{}, \r\n" + "dstIpForAudio->{}, \n" + "dstIpForAudio->{}, \n" + "dstPortForAudio->{},  \n" + "dstPortForVideo->{}, \n" + "app->{}, \n" + "stream->{}, \n" + "callId->{}, \n" + "ptForAudio->{}, \n" + "ptForVideo->{}", ssrc, dstIpForAudio, dstIpForVideo, dstPortForAudio, dstPortForVideo, app, stream, callId, ptForAudio, ptForVideo);
        if (!((dstPortForAudio > 0 && !ObjectUtils.isEmpty(dstPortForAudio) || (dstPortForVideo > 0 && !ObjectUtils.isEmpty(dstIpForVideo))))) {
            throw new ServiceException("至少应该存在一组音频或视频发送参数");
        }
        MediaServer mediaServer = mediaServerService.getDefaultMediaServer();
        String key = VideoManagerConstants.WVP_OTHER_SEND_RTP_INFO + userSetting.getServerId() + "_" + callId;
    ...
    @Parameter(name = "callId", description = "整个过程的唯一标识，不传则使用随机端口发流", required = true)
    public R<Void> closeSendRtp(String callId) {
