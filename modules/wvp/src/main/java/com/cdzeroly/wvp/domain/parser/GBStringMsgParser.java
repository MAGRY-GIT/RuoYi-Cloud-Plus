package com.cdzeroly.wvp.domain.parser;
...
public class GBStringMsgParser implements MessageParser {
    ...
    /**
     * 解析包含单个 SIP 消息的缓冲区，其中正文是未解释的字节数组。这是为了在缓冲区合并了 Callcast 的 Will Sullin 注意到的错误修复
     *
     * @param msgBuffer 包含要解析的消息的字节缓冲区。这可以由连接在一起的多个 SIP 消息组成。
     * @return 包含已解析的 SIP 消息的 SIPMessage[] 结构（请求或响应）。
     * @throws ParseException 在遇到非法消息时抛出（并且缓冲区的其余部分将被丢弃）
    ...
    public SIPMessage parseSIPMessage(byte[] msgBuffer, boolean readBody, boolean strict, ParseExceptionListener parseExceptionListener) throws ParseException {
        ...
        if (readBody && message.getContentLength() != null) {
            ...
            } else if ("MESSAGE".equalsIgnoreCase(message.getCSeqHeader().getMethod())) {
                int bodyLength = msgBuffer.length - i;
     ...
     *
     * @param telephoneNumber is a String containing the telephone # to be parsed
     * @return a parsed address structure.
     * @throws ParseException a ParseException when the address is badly formatted.
     * @since v1.0
     */
    public TelephoneNumber parseTelephoneNumber(String telephoneNumber) throws ParseException {
        // Bug fix contributed by Will Scullin
        return new URLParser(telephoneNumber).parseTelephoneNumber(true);
