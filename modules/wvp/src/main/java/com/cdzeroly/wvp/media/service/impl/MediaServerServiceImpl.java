package com.cdzeroly.wvp.media.service.impl;
...
import com.cdzeroly.wvp.storager.mapper.MediaServerMapper;
...
public class MediaServerServiceImpl implements IMediaServerService {
    ...
    private final Map<String, IMediaNodeServerService> nodeServerServiceMap;
    ...
    public void updateVmServer(List<MediaServer> mediaServerList) {
        ...
        for (MediaServer mediaServer : mediaServerList) {
            ...
            if (!hasKey) {
                RedisUtils.setCacheMapValue(key, mediaServer.getId(), mediaServer);
            }
        }
    }
    ...
    public void update(MediaServer mediaServer) {
        //更新数据库
        mediaServerMapper.insertOrUpdate(mediaServer);
        // 缓存获取媒体服务器信息
        MediaServer mediaServerInRedis = getOne(mediaServer.getId());
        // 获取完整数据
        MediaServer mediaServerInDataBase = mediaServerMapper.selectById(mediaServer.getId());
        if (mediaServerInDataBase == null) {
            return;
        }
        mediaServerInDataBase.setStatus(mediaServer.isStatus());
        //缓存为空或名称不存在则初始化
        ...
        String key = VideoManagerConstants.MEDIA_SERVER_PREFIX + userSetting.getServerId();
        RedisUtils.setCacheMapValue(key, mediaServerInDataBase.getId(), mediaServerInDataBase);

        if (mediaServerInDataBase.isStatus()) {
            resetOnlineServer(mediaServerInDataBase);
        }
    }
    ...
    public List<MediaServer> getAll() {
        List<MediaServer> mediaServerList = mediaServerMapper.selectList();
    ...
    public List<MediaServer> getAllFromDatabase() {
        return mediaServerMapper.selectList();
    }
    ...
    public MediaServer getOne(String mediaServerId) {
        ...
        String key = VideoManagerConstants.MEDIA_SERVER_PREFIX + userSetting.getServerId();
        return RedisUtils.getCacheMapValue(key,mediaServerId);
    }


    @Override
    public MediaServer getDefaultMediaServer() {
        LambdaQueryWrapper<MediaServer> wrapper = Wrappers.lambdaQuery();
        //TODO 后续看是否报错
        wrapper.eq(MediaServer::isDefaultServer,true);
        return mediaServerMapper.selectOne(wrapper);
    }
    ...
    public void add(MediaServer mediaServer) {
        ...
        if (mediaServerMapper.selectById(mediaServer.getId()) != null) {
            log.info("[添加媒体节点] 失败, 媒体服务ID已存在，请修改媒体服务器配置, {}", mediaServer.getId());
            throw new ServiceException("保存失败，媒体服务ID [ " + mediaServer.getId() + " ] 已存在，请修改媒体服务器配置");
        }
        IMediaNodeServerService mediaNodeServerService = nodeServerServiceMap.get(mediaServer.getType());
        if (mediaNodeServerService == null) {
    ...
    @Override
    public void resetOnlineServer(MediaServer mediaServer) {
        // 更新缓存
        String key = VideoManagerConstants.ONLINE_MEDIA_SERVERS_PREFIX + userSetting.getServerId();
        // 使用zset的分数作为当前并发量， 默认值设置为0
        // 不存在则设置默认值 已存在则重置
        if (redisTemplate.opsForZSet().score(key, mediaServer.getId()) == null) {
            redisTemplate.opsForZSet().add(key, mediaServer.getId(), 0L);
            // 查询服务流数量
            int count = getMediaList(mediaServer);
            redisTemplate.opsForZSet().add(key, mediaServer.getId(), count);
        }else {
            clearRTPServer(mediaServer);
        }
    }
    ...
    public MediaServer checkMediaServer(String ip, int port, String secret, String type) {
        LambdaQueryWrapper<MediaServer> wrapper = Wrappers.lambdaQuery();
        wrapper.eq(MediaServer::getIp,ip);
        wrapper.eq(MediaServer::getHttpPort,port);

        if (mediaServerMapper.exists(wrapper)) {
            throw new ServiceException( "此连接已存在");
        }
        ...
        if (mediaServer != null) {
            if (mediaServerMapper.selectById(mediaServer.getId()) != null) {
                throw new ServiceException( "媒体服务ID [" + mediaServer.getId() + " ] 已存在，请修改媒体服务器配置");
            }
        }
        return mediaServer;
    }
    ...
    @Override
    public MediaServer getMediaServer(String mediaServerId) {
    ...
    public void syncCatchFromDatabase() {
        ...
        List<MediaServer> allInDatabase = mediaServerMapper.selectList();
    ...
    @Override
    public MediaServerLoadVo getLoad(MediaServer mediaServer) {
        MediaServerLoadVo result = new MediaServerLoadVo();
    ...
    public List<MediaServer> getAllWithAssistPort() {
        LambdaQueryWrapper<MediaServer> wrapper = Wrappers.lambdaQuery();
        wrapper.ge(MediaServer::getRecordAssistPort,0);
        return mediaServerMapper.selectList(wrapper);
    }
    ...
    public StreamInfo getStreamInfoByAppAndStreamWithCheck(String app, String stream, String mediaServerId, String addr, boolean authority) {
        ...
        if (mediaInfo == null) {
            throw new ServiceException( "未找到使用的媒体节点");
        }
        String calld = null;
    ...
    public Integer startSendRtpPassive(MediaServer mediaServer, SendRtpInfo sendRtpItem, Integer timeout) {
        ...
        if (mediaNodeServerService == null) {
            ...
            throw new ServiceException( "未找到mediaServer对应的实现类");
        }
